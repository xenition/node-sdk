...
describe('media.privateAlbum', () => {
    it('GETs the private route with the code query param and returns it merged with items', async () => {
        fetchMock.mockResolvedValue(jsonOk({ id: '2', slug: 'smith-wedding', published: false, items: [{ id: 'i1', albumId: '2' }] }));
...
describe('billing.verify', () => {
    it('POSTs the apple shape', async () => {
        ...
        expect(JSON.parse(calledInit().body)).toEqual({ platform: 'apple', transactionId: 'tx-1' });
    });
    it('POSTs the google shape and surfaces `acknowledged`', async () => {
        ...
        expect(JSON.parse(calledInit().body)).toEqual({ platform: 'google', productId: 'premium.monthly', purchaseToken: 'tok', kind: 'subscription' });
...
describe('billing.restore', () => {
    it('POSTs the apple chain id and returns the resulting entitlements', async () => {
        ...
        expect(JSON.parse(calledInit().body)).toEqual({ platform: 'apple', originalTransactionId: 'otx-1' });
    ...
    it('POSTs the google token', async () => {
        ...
        expect(JSON.parse(calledInit().body)).toEqual({ platform: 'google', productId: 'premium.monthly', purchaseToken: 'tok' });
    });
});
...
describe('accessToken', () => {
    const authed = (token) => (0, app_client_1.createAppClient)('/api', { accessToken: token });
    it('attaches the bearer header to a GET', async () => {
        fetchMock.mockResolvedValue(jsonOk({ entitlements: [] }));
        await authed('tok-1').billing.entitlements();
        expect(calledInit().headers.Authorization).toBe('Bearer tok-1');
    });
    ...
    it('reads the token per request, so a refresh is picked up', async () => {
        let token = 'first';
        const client = (0, app_client_1.createAppClient)('/api', { accessToken: () => token });
    ...
    it('awaits an async resolver (device storage)', async () => {
        ...
        const client = (0, app_client_1.createAppClient)('/api', { accessToken: async () => 'stored' });
        expect(await client.notifications.unreadCount()).toBe(3);
        expect(calledInit().headers.Authorization).toBe('Bearer stored');
    });
    it('sends no header at all when the resolver has no token (signed out)', async () => {
        fetchMock.mockResolvedValue(jsonOk({ products: [] }));
        await (0, app_client_1.createAppClient)('/api', { accessToken: () => null }).billing.products();
        expect(fetchMock.mock.calls[0][1]).toBeUndefined();
    });
});
/* ============================= auth ============================= */
describe('auth', () => {
    it('POSTs /auth/login and returns the session', async () => {
        ...
        expect(JSON.parse(calledInit().body)).toEqual({ email: 'a@b.co', password: 'pw' });
    ...
    it('POSTs /auth/refresh with the refresh token in the body', async () => {
        ...
        expect(JSON.parse(calledInit().body)).toEqual({ refreshToken: 'r1' });
    });
    it('POSTs the id token to the provider route', async () => {
        ...
        expect(JSON.parse(calledInit().body)).toEqual({ idToken: 'id-1', nonce: 'n' });
    });
    ...
    it('DELETEs the account and surfaces the grace period', async () => {
        ...
        expect(calledInit().method).toBe('DELETE');
        expect(JSON.parse(calledInit().body)).toEqual({ password: 'pw' });
...
describe('notifications.savePreferences', () => {
    it('PUTs the camelCase patch and returns EVERY row it touched', async () => {
        ...
        expect(JSON.parse(calledInit().body)).toEqual({ quietStartMinute: 1320, quietEndMinute: 420 });
    ...
    it('sends null through as "clear the window" rather than dropping it', async () => {
        ...
        expect(JSON.parse(calledInit().body)).toEqual({ category: 'reminder', quietStartMinute: null, quietEndMinute: null });
    });
});
...
describe('402', () => {
    ...
    const errBody = (status, body) => ({ ok: false, status, json: async () => body });
    /**
    ...
    it('lifts the entitlement KEY and the gate full check', async () => {
        ...
        expect(err).toBeInstanceOf(errors_1.AppClientError);
    ...
    it('narrows an unknown catch with isPaymentRequired()', async () => {
        ...
        try {
            ...
            expect((0, errors_1.isPaymentRequired)(err)).toBe(true);
            if ((0, errors_1.isPaymentRequired)(err))
                expect(err.quota.used).toBe(5);
        }
        expect((0, errors_1.isPaymentRequired)(new Error('nope'))).toBe(false);
    });
});
describe('formatDate', () => {
