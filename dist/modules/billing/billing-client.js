...
exports.billingModule = exports.BillingClient = exports.BILLING_MIGRATIONS = exports.BILLING_TABLES = void 0;
exports.isExpired = isExpired;
exports.daysUntil = daysUntil;
const core_1 = require("../core");
const util_1 = require("../util");
exports.BILLING_TABLES = {
    PRODUCTS: 'billing__products',
    PURCHASES: 'billing__purchases',
    ENTITLEMENTS: 'billing__entitlements',
    EVENTS: 'billing__events',
};
exports.BILLING_MIGRATIONS = [
    {
        id: 'billing/0001_create_billing__products',
        sql: `CREATE TABLE IF NOT EXISTS ${exports.BILLING_TABLES.PRODUCTS} (
  id uuid PRIMARY KEY,
  ...
  ON ${exports.BILLING_TABLES.PRODUCTS} (platform, product_id)`,
    },
    {
        id: 'billing/0003_create_billing__purchases',
        sql: `CREATE TABLE IF NOT EXISTS ${exports.BILLING_TABLES.PURCHASES} (
  id uuid PRIMARY KEY,
  ...
  ON ${exports.BILLING_TABLES.PURCHASES} (platform, original_transaction_id)`,
    },
    {
        id: 'billing/0005_index_billing__purchases_user',
        sql: `CREATE INDEX IF NOT EXISTS billing__purchases_user_idx
  ON ${exports.BILLING_TABLES.PURCHASES} (user_id, status)`,
    },
    {
        id: 'billing/0006_create_billing__entitlements',
        sql: `CREATE TABLE IF NOT EXISTS ${exports.BILLING_TABLES.ENTITLEMENTS} (
  id uuid PRIMARY KEY,
  user_id text NOT NULL,
  entitlement text NOT NULL,
  ...
  ON ${exports.BILLING_TABLES.ENTITLEMENTS} (user_id, entitlement)`,
    },
    {
        id: 'billing/0008_create_billing__events',
        sql: `CREATE TABLE IF NOT EXISTS ${exports.BILLING_TABLES.EVENTS} (
  id uuid PRIMARY KEY,
  platform text NOT NULL CHECK (platform IN ('apple', 'google', 'stripe')),
  ...
  ON ${exports.BILLING_TABLES.EVENTS} (platform, event_id)`,
    },
];
const PLATFORMS = ['apple', 'google', 'stripe'];
const PRODUCT_KINDS = ['subscription', 'non_consumable', 'consumable'];
...
const ENTITLEMENT_SOURCES = ['purchase', 'trial', 'grant'];
/**
 ...
 */
const ACCESS_GRANTING = ['active', 'grace'];
...
class BillingClient {
     ...
     */
    async defineProduct(input) {
        const context = 'BillingClient.defineProduct';
        const productId = (0, util_1.requireNonEmptyString)(context, 'productId', input?.productId);
        const platform = this.requirePlatform(context, input?.platform);
        const entitlement = (0, util_1.requireNonEmptyString)(context, 'entitlement', input?.entitlement);
        ...
        if (period !== null && typeof period !== 'string') {
            (0, util_1.fail)(context, '"period" must be a string or null');
        }
        const active = input?.active ?? true;
        if (typeof active !== 'boolean')
            (0, util_1.fail)(context, '"active" must be a boolean');
        const existing = await this.findProduct(platform, productId);
        if (existing) {
            await this.ctx.query
                .from(exports.BILLING_TABLES.PRODUCTS)
        ...
        await this.ctx.query.from(exports.BILLING_TABLES.PRODUCTS).insert(row).execute();
        return product;
    }
    async listProducts(options = {}) {
        let q = this.ctx.query.from(exports.BILLING_TABLES.PRODUCTS);
        if (options.platform)
            q = q.where('platform', options.platform);
        if (options.entitlement)
            q = q.where('entitlement', options.entitlement);
        if (options.active !== 'all')
            q = q.where('active', options.active ?? true);
        return q.orderBy('product_id', 'ASC').rows();
    }
    /** The product record for a store id, or null when it was never declared. */
    async findProduct(platform, productId) {
    ...
    async recordPurchase(input) {
        ...
        const platform = this.requirePlatform(context, input?.platform);
        const productId = (0, util_1.requireNonEmptyString)(context, 'productId', input?.productId);
        const originalTransactionId = (0, util_1.requireNonEmptyString)(context, 'originalTransactionId', input?.originalTransactionId);
        const transactionId = (0, util_1.requireNonEmptyString)(context, 'transactionId', input?.transactionId ?? originalTransactionId);
        const status = this.requireStatus(context, input?.status ?? 'active');
        const raw = (0, util_1.optionalPlainObject)(context, 'raw', input?.raw, {});
        ...
        if (existing) {
            ...
            await this.ctx.query
                .from(exports.BILLING_TABLES.PURCHASES)
                .update({ ...fields, user_id: owner })
                .where('id', existing.id)
                .execute();
            purchase = { ...existing, ...fields, user_id: owner };
        }
        else {
            purchase = {
                id: (0, util_1.generateId)(),
            ...
            await this.ctx.query.from(exports.BILLING_TABLES.PURCHASES).insert(row).execute();
        }
        await this.projectEntitlement(purchase);
        return purchase;
    }
    /** The purchase for a store transaction chain, or null. */
    async findPurchase(platform, originalTransactionId) {
        const row = await this.ctx.query
            .from(exports.BILLING_TABLES.PURCHASES)
            .where('platform', platform)
    ...
    async listPurchases(userId, options = {}) {
        const context = 'BillingClient.listPurchases';
        (0, util_1.requireNonEmptyString)(context, 'userId', userId);
        let q = this.ctx.query.from(exports.BILLING_TABLES.PURCHASES).where('user_id', userId);
        if (options.status)
            q = q.where('status', this.requireStatus(context, options.status));
        if (options.platform)
            q = q.where('platform', this.requirePlatform(context, options.platform));
     ...
     * run.
     */
    async check(userId, entitlement) {
        const context = 'BillingClient.check';
        (0, util_1.requireNonEmptyString)(context, 'userId', userId);
        const key = (0, util_1.requireNonEmptyString)(context, 'entitlement', entitlement);
    ...
    async listEntitlements(userId) {
        ...
        const rows = await this.ctx.query
            .from(exports.BILLING_TABLES.ENTITLEMENTS)
    ...
    async grant(input) {
        ...
        if (expiresAt !== null && typeof expiresAt !== 'string') {
            (0, util_1.fail)(context, '"expiresAt" must be an ISO timestamp string or null');
        }
    ...
    async startTrial(input) {
        ...
        const userId = (0, util_1.requireNonEmptyString)(context, 'userId', input?.userId);
        const entitlement = (0, util_1.requireNonEmptyString)(context, 'entitlement', input?.entitlement);
        const days = (0, util_1.optionalNumber)(context, 'days', input?.days, 7);
        if (!Number.isFinite(days) || days <= 0) {
            (0, util_1.fail)(context, '"days" must be a positive number');
        }
    ...
    async revoke(userId, entitlement) {
        const context = 'BillingClient.revoke';
        (0, util_1.requireNonEmptyString)(context, 'userId', userId);
        (0, util_1.requireNonEmptyString)(context, 'entitlement', entitlement);
        await this.ctx.query
            .from(exports.BILLING_TABLES.ENTITLEMENTS)
            .update({ status: 'revoked', updated_at: (0, util_1.nowIso)() })
            .where('user_id', userId)
            .where('entitlement', entitlement)
            .execute();
    }
    async findEntitlement(userId, entitlement) {
        const row = await this.ctx.query
            .from(exports.BILLING_TABLES.ENTITLEMENTS)
            .where('user_id', userId)
            .where('entitlement', entitlement)
    ...
    async recordEvent(input) {
        ...
        const platform = this.requirePlatform(context, input?.platform);
        const eventId = (0, util_1.requireNonEmptyString)(context, 'eventId', input?.eventId);
        const notificationType = (0, util_1.requireNonEmptyString)(context, 'notificationType', input?.notificationType);
        if (await this.findEvent(platform, eventId))
            return false;
        await this.ctx.query
            .from(exports.BILLING_TABLES.EVENTS)
            .insert({
            id: (0, util_1.generateId)(),
            platform,
            ...
            event_id: eventId,
            payload: (0, util_1.optionalPlainObject)(context, 'payload', input?.payload, {}),
        })
            .execute();
        return true;
    }
    /** A previously recorded notification, or null. */
    async findEvent(platform, eventId) {
        const row = await this.ctx.query
            .from(exports.BILLING_TABLES.EVENTS)
            .where('platform', platform)
     ...
     * out of a store id.
     */
    async projectEntitlement(purchase) {
        ...
        const status = purchase.status === 'refunded' || purchase.status === 'revoked'
            ? 'revoked'
            : granting
                ? 'active'
                : 'expired';
    ...
    async upsertEntitlement(input) {
        ...
        await this.ctx.query.from(exports.BILLING_TABLES.ENTITLEMENTS).insert(row).execute();
        return record;
    }
    requirePlatform(context, value) {
        if (!PLATFORMS.includes(value)) {
            (0, util_1.fail)(context, `"platform" must be one of ${PLATFORMS.join(', ')}`);
        }
        return value;
    }
    requireKind(context, value) {
        if (!PRODUCT_KINDS.includes(value)) {
            (0, util_1.fail)(context, `"kind" must be one of ${PRODUCT_KINDS.join(', ')}`);
        }
        return value;
    }
    requireStatus(context, value) {
        if (!PURCHASE_STATUSES.includes(value)) {
            (0, util_1.fail)(context, `"status" must be one of ${PURCHASE_STATUSES.join(', ')}`);
        }
        return value;
    }
    requireSource(context, value) {
        if (!ENTITLEMENT_SOURCES.includes(value)) {
            (0, util_1.fail)(context, `"source" must be one of ${ENTITLEMENT_SOURCES.join(', ')}`);
        }
        return value;
    }
}
...
exports.billingModule = (0, core_1.defineModule)({
    name: 'billing',
    migrations: exports.BILLING_MIGRATIONS,
    factory: (ctx) => new BillingClient(ctx),
});
