...
class StorageClient {
    ...
    async createUploadUrl(path, options = {}) {
        try {
            ...
            if (err instanceof errors_1.XenitionError && (err.code === 'NOT_FOUND' || err.code === 'NOT_IMPLEMENTED')) {
                throw new errors_1.XenitionError(err.code, `StorageClient.createUploadUrl("${path}"): this gateway cannot issue upload URLs ` +
                    '(an older gateway, or an object store that cannot presign). Use upload() to ' +
                    'send the bytes through your server instead.', { status: err.status, details: err.details });
            }
            throw err;
        }
    }
