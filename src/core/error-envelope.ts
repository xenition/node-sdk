 ...
 */
export function messageFromEnvelope(envelope: unknown): string | undefined {
  ...
  if (Array.isArray(env.message)) {
    const lines = env.message.filter((m): m is string => typeof m === 'string' && m.trim() !== '');
 ...
 * the error instead.
 */
export function codeFromEnvelope(envelope: unknown): string | undefined {
  if (!envelope || typeof envelope !== 'object') return undefined;
  const env = envelope as ErrorEnvelope;
  if (env.error && typeof env.error === 'object') return env.error.code;
  return undefined;
}
