...
export const EVENTS_MIGRATIONS: Migration[] = [
  {
    ...
    sql: `CREATE TABLE IF NOT EXISTS ${EVENTS_TABLES.EVENTS} (
  id uuid PRIMARY KEY,
  ...
  },
  {
    ...
    sql: `CREATE INDEX IF NOT EXISTS events__events_status_starts_at_idx ON ${EVENTS_TABLES.EVENTS} (status, starts_at)`,
  },
  {
    id: 'events/0003_create_events__rsvps',
    sql: `CREATE TABLE IF NOT EXISTS ${EVENTS_TABLES.RSVPS} (
  id uuid PRIMARY KEY,
  ...
  },
  {
    ...
    sql: `CREATE INDEX IF NOT EXISTS events__rsvps_event_status_idx ON ${EVENTS_TABLES.RSVPS} (event_id, status)`,
  },
];

const EVENT_STATUSES: EventStatus[] = ['draft', 'published', 'cancelled'];
const RSVP_STATUSES: RsvpStatus[] = ['confirmed', 'waitlist', 'cancelled'];
const WHENS: EventWhen[] = ['upcoming', 'past', 'all'];
...
export class EventsClient {
  ...
  async create(input: CreateEventInput): Promise<EventRecord> {
    ...
    const endsAt =
      input.endsAt === undefined ? null : this.requireIso(context, 'endsAt', input.endsAt);
    ...
    if (ends_at !== null) row.ends_at = ends_at;
    await this.ctx.query.from(EVENTS_TABLES.EVENTS).insert(row).execute();
    return event;
  }
   ...
   */
  async list(options: ListEventsOptions = {}): Promise<EventRecord[]> {
    const context = 'EventsClient.list';
    const when = options.when ?? 'upcoming';
    if (!WHENS.includes(when)) {
      fail(context, `"when" must be one of ${WHENS.join(', ')} — got "${String(when)}"`);
    }
    const status = options.status ?? 'published';

    let qb = this.ctx.query.from(EVENTS_TABLES.EVENTS);
    if (status !== 'all') {
      if (!EVENT_STATUSES.includes(status as EventStatus)) {
        fail(context, `"status" must be one of ${EVENT_STATUSES.join(', ')}, all — got "${String(status)}"`);
      }
      qb = qb.where('status', status);
    }
    const now = nowIso();
    if (when === 'upcoming') qb = qb.where('starts_at', '>=', now).orderBy('starts_at', 'ASC');
    else if (when === 'past') qb = qb.where('starts_at', '<', now).orderBy('starts_at', 'DESC');
    else qb = qb.orderBy('starts_at', 'ASC');
   ...
   */
  async rsvp(slugOrId: string, input: RsvpInput): Promise<Rsvp> {
    ...
    if (event.status === 'cancelled') {
      fail(context, `event "${event.slug}" is cancelled and is not accepting RSVPs`);
    }
    ...
    if (capacity > 0) {
      ...
      if (confirmedSeats + partySize > capacity) status = 'waitlist';
    }
    ...
    await this.ctx.query.from(EVENTS_TABLES.RSVPS).insert(row).execute();
    return rsvp;
  }
  ...
  async listRsvps(eventId: string, options: ListRsvpsOptions = {}): Promise<Rsvp[]> {
    ...
    requireNonEmptyString(context, 'eventId', eventId);
    let qb = this.ctx.query.from(EVENTS_TABLES.RSVPS).where('event_id', eventId);
    if (options.status !== undefined) {
      if (!RSVP_STATUSES.includes(options.status)) {
        fail(context, `"status" must be one of ${RSVP_STATUSES.join(', ')} — got "${String(options.status)}"`);
      }
      qb = qb.where('status', options.status);
    }
    return qb.orderBy('created_at', 'DESC').rows<Rsvp>();
  }

  /** Cancel an RSVP by id (flips status to 'cancelled'). */
  async cancelRsvp(id: string): Promise<void> {
   ...
   * client-side.
   */
  private async counts(eventId: string, capacity: number): Promise<EventCounts> {
  ...
  private confirmedSeats(eventId: string): Promise<number> {
    return this.seatSum(eventId, 'confirmed');
  }

  private async seatSum(eventId: string, status: RsvpStatus): Promise<number> {
    ...
    return rows.reduce(
      (sum, r) => sum + (toNumber(r?.party_size ?? r?.partySize) ?? 0),
      0,
    );
  }

  private async resolveEvent(context: string, slugOrId: string): Promise<EventRecord> {
    requireNonEmptyString(context, 'slugOrId', slugOrId);
  ...
  private validatePartySize(context: string, value: unknown): number {
    const n = optionalNumber(context, 'partySize', value, 1);
    if (!Number.isInteger(n) || n < MIN_PARTY || n > MAX_PARTY) {
      fail(context, `"partySize" must be an integer between ${MIN_PARTY} and ${MAX_PARTY}`);
    }
    return n;
  }

  private validateCapacity(context: string, value: unknown): number {
    const n = optionalNumber(context, 'capacity', value, 0);
    if (!Number.isInteger(n) || n < 0) {
      fail(context, '"capacity" must be a non-negative integer (0 = unlimited)');
    }
    return n;
  }

  private validateStatus(context: string, value: unknown, fallback: EventStatus): EventStatus {
    if (value === undefined) return fallback;
    if (typeof value !== 'string' || !EVENT_STATUSES.includes(value as EventStatus)) {
      fail(context, `"status" must be one of ${EVENT_STATUSES.join(', ')} — got "${String(value)}"`);
    }
...
export const eventsModule = defineModule({
  ...
  factory: (ctx: ModuleContext) => new EventsClient(ctx),
});
