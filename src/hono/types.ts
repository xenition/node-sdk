...
export type XenitionApiModule =
  // Not a data module — mounts the end-user account router over
  // `client.auth`: sign-in, sessions, profile, deletion and export.
  | 'auth'
  | 'cms'
  | 'forms'
  | 'reviews'
  ...
  | 'jobs'
  // The in-app inbox: feed, badge count, read state, preferences.
  | 'notifications'
  // Usage meters, read-only — spending a quota stays server-side.
  | 'quotas';
